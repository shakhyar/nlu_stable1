...
from nlu.etc.model.inference import chat
from nlu.etc.model.config import *
				...
				if self.query:
					...
					if not self.intent: # if no intent associated, self.intent is assigned as None
						...
						if '--weather' == self.intent:
							...
							if 'humid' in self.infered_list[0] or if 'sweat' in self.infered_list[0]:
								tts.say(current_humidity)

							elif 'weather' in self.infered_list[0] or if 'cloud' in self.infered_list[0] or if 'rain' in self.infered_list[0] or if 'windy' in self.infered_list[0] or if 'sunnny' self.infered_list[0]:
